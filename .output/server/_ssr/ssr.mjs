...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-CSZZaUSb.mjs").then((n) => n.t).then((m) => m.default ?? m);
...
async function proxyTts(request) {
	...
	if (!response.ok) return new Response("TTS service unavailable", { status: 502 });
