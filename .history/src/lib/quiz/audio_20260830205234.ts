...
export class AudioEngine {
  ...
  /**
   * Play a sound effect. `delay` (seconds) schedules it on the Web Audio
   * clock ahead of time, so it fires at exactly the right instant even if
   * the main thread is busy or briefly blocked when `delay` elapses —
   * scheduling is handled by the audio hardware clock, not a JS timer.
   */
