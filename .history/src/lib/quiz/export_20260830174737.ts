...
/**
 * Real seconds a narrated beat's trailing decorative SFX needs to actually
 * play and get captured before we snapshot the beat's recorded audio. This
 * is the only real-time wait left for narrated beats beyond speech itself,
 * and it's small.
 */
...
/** One beat's worth of final audio, placed at an absolute offset into the
 *  overall render timeline. */
 ...
 * letting it play through the tab's audio and grabbing it via
 ...
 * loop is NOT tied to requestAnimationFrame or wall-clock time — if the
 * CPU is slow, generating those frames just takes longer, it can never
 * skip or duplicate one.
 *
...
export async function renderVideo(opts: {
  ...
  // thread — immune to main-thread stalls, unlike MediaRecorder+
  // captureStream. Fixed beats never touch this capture; their audio is
  // rendered offline instead (see renderBeatAudioOffline above).
