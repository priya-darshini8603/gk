...
export class AudioEngine {
  ...
  get hasNarrationCapture() {
    ...
    /** Total PCM samples captured so far — used as the ground-truth clock
   * for beat durations during export (see export.ts). Immune to main
   * thread stalls since capture runs on the AudioWorklet thread. */
  get pcmSamplesCaptured() {
    return this.pcmSampleCount;
  }

  /** Sample rate of the live AudioContext, if one exists yet. */
  get sampleRate() {
