...
export class AudioEngine {
  ...
  /**
   * Speak using MICROSOFT ZIRA ONLY.
   *
   * Zira is the ONLY permitted voice.
   *
   * IMPORTANT:
   * - No en-IN voice selection.
   * - No Google voice fallback.
   * - No Microsoft Indian voice fallback.
   * - No browser default fallback.
   * - If Zira is unavailable, narration fails.
   *
   * Resolves with the ACTUAL spoken duration.
   */
  async speak(
    text: string,
    language: Language,
    ...
    if (
      ...
      console.warn(
        "[audio] speechSynthesis unsupported."
      );
    ...
    /*
     * Wait for Chrome/browser voices to become available
     * before searching for Microsoft Zira.
     */
     ...
     *
     * Only Microsoft Zira is accepted.
     */
    const ziraVoice =
      voices.find(
        (v) =>
    ...
    /*
     * Extra fallback match for systems that expose
     * the voice as "Zira" without the Microsoft prefix.
     *
     * This is STILL Zira only.
     */
    const zira =
      ziraVoice ??
      voices.find(
    ...
    if (!zira) {
      console.error(
        "[audio] ❌ MICROSOFT ZIRA NOT AVAILABLE."
      );
      ...
      /*
       * Do NOT allow SpeechSynthesis to use
       * the browser's default voice.
       */
      throw new Error(
        "Microsoft Zira voice is required but is not available on this system."
      );
    }
     ...
     * Force Zira.
     *
     * Do NOT use en-IN here.
     * Zira is normally exposed as en-US.
     */
    u.voice = zira;
    u.lang =
      zira.lang || "en-US";
    ...
    return new Promise<number>(
      (resolve) => {
         ...
         * Safety timeout only.
         *
         * Normal duration comes from the
         * speechSynthesis "end" event.
         */
        const fallbackMs =
          Math.max(
            4000,
         ...
         * IMPORTANT:
         *
         * u.voice is explicitly set to Zira.
         * Therefore browser default voice
         * selection cannot occur normally.
         */
  ...
  /**
   * Wait until current narration finishes.
   * Kept as a standalone utility.
   */
  waitForSpeechEnd(
    timeoutMs = 8000
  ): Promise<void> {
