...
/**
 * Real-time "wait" phase for a fixed-duration beat. This only exists to
 * pace SFX playback (oscillators scheduled on the AudioContext clock, which
 * runs independently of the main thread) — it does NOT gate or influence
 * video frame generation, which happens afterward and is fully
 * deterministic. No canvas drawing, no rAF, nothing here can desync the
 * final video.
 */
async function waitFixedBeat(beat: Beat, audio: AudioEngine) {
...
/**
 * Generate and encode the exact frame count for one beat's time range.
 * Fully deterministic: frame count, per-frame timestamps, and per-frame
 * content are all derived from math (beatStart, beatDur, fps), never from
 * how long drawFrame() actually took to execute on this machine.
 */
async function encodeBeatFrames(params: {
  ...
  const frameCount = Math.max(1, Math.round(beatDur * fps));
  ...
  for (let i = 0; i < frameCount; i++) {
    ...
    // Backpressure only — a memory-safety valve, NOT a correctness
    // mechanism. It never skips a frame or a timestamp; it just pauses
    // briefly so the encoder's internal queue doesn't grow unbounded on a
    // very long export. Every frame index from 0..frameCount-1 is always
    // encoded, in order, with its exact timestamp.
    ...
    // Yield to the event loop occasionally so the tab stays responsive
    // (progress bar, UI). This can never change WHICH frames get encoded —
    // only how the work is paced in real wall-clock time.
    if (i % 6 === 0) {
      await sleep(0);
    }
  }
...
export async function renderVideo(opts: {
  ...
  /* -----------------------------------------------------------------------
   * BEAT LOOP — sequential per-beat: (1) real-time phase to learn/pace the
   * beat's actual duration, (2) fully deterministic frame synthesis +
   * encode for that exact duration.
   * --------------------------------------------------------------------- */
  ...
  for (let index = 0; index < total; index++) {
    ...
    beat.start = cursor;
    const percent = 6 + Math.round((index / total) * 80);
    ...
    if (beat.say) {
      ...
      beatDur = Math.max(0.25, spokenSeconds > 0 ? spokenSeconds : estimatedFallbackDur);
      beat.dur = beatDur;
      ...
      beatDur = beat.dur;
      await waitFixedBeat(beat, audio);
    ...
    onProgress({ stage: "Rendering beats...", percent: 6 + Math.round(((index + 0.5) / total) * 80) });
