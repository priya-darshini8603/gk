...
export async function renderVideo(opts: {
  ...
  // NOTE: voice loading is handled automatically inside audio.speakAndWait()
  // (it lazily loads the mespeak engine + the needed language voice on first
  // use), so there's nothing to pre-wait-for here.
