...
export function drawOwl(
  ...
  const drawWing = (side: 1 | -1, angle: number) => {
    ...
    const handUp = side === -1 ? look.handLUp : look.handRUp;
    if (handUp > 0 || angle > 1.2) {
      const handX = side * len * 0.13;
      const handY = len * 0.92;
      const spread = size * (0.025 + handUp * 0.018);
      ctx.save();
      ctx.translate(handX, handY);
      ctx.rotate(side * handUp * 0.18);
      ctx.fillStyle = "#d8953d";
      ctx.beginPath();
      ctx.ellipse(0, size * 0.018, size * 0.045, size * 0.06, 0, 0, TAU);
      ctx.fill();
      ctx.strokeStyle = "#a96525";
      ctx.lineWidth = size * 0.014;
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      [-1, 0, 1].forEach((finger) => {
        const startX = finger * spread * 0.45;
        const endX = side * size * (0.085 + Math.abs(finger) * 0.018);
        const endY = size * (0.09 + Math.abs(finger) * 0.018);
        ctx.beginPath();
        ctx.moveTo(startX, size * 0.045);
        ctx.quadraticCurveTo(endX * 0.75, endY * 0.35, endX, endY);
        ctx.stroke();
      });
      ctx.restore();
    }
