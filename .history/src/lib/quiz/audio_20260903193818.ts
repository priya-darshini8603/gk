...
export class AudioEngine {
  ...
  async startPcmCapture(): Promise<void> {
    ...
    if (!this.workletModuleReady) {
      const blob = new Blob([AudioEngine.CAPTURE_WORKLET_SOURCE], {
  ...
  private noise(dur: number, gain = 0.25, delay = 0) {
    const ctx = this.ensure();
   ...
   * utterance's 'end' event (or a bounded fallback fires).
   *
   * KNOWN BROWSER BUG THIS WORKS AROUND:
   * Chrome/Chromium-based browsers stop actually vocalizing (and often
   * never fire 'end'/'boundary') on SpeechSynthesisUtterances that run
   * longer than ~15 seconds, or during utterances that span a period
   * where the tab is backgrounded. The documented workaround is to call
   * `speechSynthesis.pause()` immediately followed by `.resume()` every
   * few seconds while an utterance is in flight — this resets Chrome's
   * internal watchdog without audibly interrupting playback. Without
   * this, a single long/awkward narration line can silently die mid-way,
   * and callers (export.ts) that are waiting on this promise will sit
   * frozen until the hard fallback timeout finally fires — which is what
   * shows up as "hanging" frames during export.
   */
  async speak(
    text: string,
    language: Language,
    ...
    return new Promise<number>((resolve) => {
      ...
      // Generous cap: word-count estimate, tripled, with a sane floor/ceiling.
      // This is a hard backstop only — real speech almost always resolves via
      // 'onend' well before this fires.
      const fallbackMs = Math.min(
        45_000,
        Math.max(4000, text.split(/\s+/).length * 500 * 1.5)
      );
      ...
      // Chrome speech-cutoff workaround: nudge the engine every 10s so it
      // never crosses the ~15s internal watchdog threshold while speaking.
