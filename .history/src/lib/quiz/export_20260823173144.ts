...
export async function renderVideo(opts: {
  ...
  try {
    ...
    const mp4Header = new TextDecoder().decode(finalData.slice(0, Math.min(finalData.length, 2_000_000)));
    if (!mp4Header.includes("mp4a")) throw new Error("FFmpeg did not produce an AAC audio stream in the MP4.");
