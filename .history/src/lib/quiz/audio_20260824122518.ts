...
const femaleVoice =
  voices.find(v =>
    /Microsoft Zira|Microsoft Jenny|Samantha|Karen|Google US English Female|Google UK English Female|Heera|Neerja/i.test(v.name) &&
    v.lang.startsWith(locale.split("-")[0]!)
  ) ??
  voices.find(v =>
    /Zira|Jenny|Samantha|Karen|Heera|Neerja|Female/i.test(v.name) &&
    v.lang.startsWith(locale.split("-")[0]!)
  );

if (femaleVoice) {
  u.voice = femaleVoice;
  console.log("[audio] Female voice selected:", femaleVoice.name, femaleVoice.lang);
} else {
  console.warn("[audio] No female voice found for", locale);
}
