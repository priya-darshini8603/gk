...
export function buildTimeline(
  ...
  if (isFirstQuestion) {
    ...
    const intro = pick([
      ...
      "Hello friends! Let's play a quiz game together!",
    ]);
...
/**
 * Deep-copies a Timeline so a renderer can safely stretch beat durations
 * (see extendBeatDuration) without mutating the shared timeline object that
 * the interactive preview/editor also holds a reference to.
 */
export function cloneTimeline(tl: Timeline): Timeline {
  return {
    seed: tl.seed,
    duration: tl.duration,
    beats: tl.beats.map((b) => ({ ...b })),
  };
}

/**
 * Stretches a beat to at least `minDur` seconds and shifts every subsequent
 * beat's start time (plus the overall timeline duration) forward by the
 * same delta.
 *
 * Used by the video renderer when actual spoken narration (measured via
 * SpeechSynthesisUtterance.onend) runs longer than the word-count estimate
 * that originally sized the beat — e.g. a long explanation or end-card
 * line. This keeps the timeline's beats/start/duration data internally
 * consistent with what was actually rendered, without ever dropping,
 * overlapping, or truncating a beat.
 */
export function extendBeatDuration(
  timeline: Timeline,
  beatIndex: number,
  minDur: number,
): void {
  ...
  if (!beat || minDur <= beat.dur) {
    return;
  }

  const delta = minDur - beat.dur;

  beat.dur = minDur;

  for (let i = beatIndex + 1; i < timeline.beats.length; i++) {
    timeline.beats[i]!.start += delta;
  }

  timeline.duration += delta;
}