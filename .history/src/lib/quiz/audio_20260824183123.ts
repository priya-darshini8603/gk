...
export class AudioEngine {
  ...
  // The utterance currently in flight, if any. Used so stopSpeech()/dispose()
  // can cleanly cancel whatever speak() is awaiting.
  ...
  async captureNarration(): Promise<boolean> {
    ...
    this.captureReady = (async () => {
      ...
      try {
        ...
        this.captureSource = ctx.createMediaStreamSource(
          this.captureStream
        );
  ...
  /**
   * Speak using an Indian English female voice.
   *
   * Priority:
   ...
   *
   * IMPORTANT (fixed): this promise now resolves ONLY when the browser
   * fires the utterance's `onend` (or `onerror`) event. It previously
   * resolved ~50ms after calling speak(), which meant callers had no real
   * signal that narration had finished — long explanations/end-card lines
   * were getting cut off by a separate hardcoded timeout in export.ts.
   *
   * A pause/resume "keep-alive" nudge is used while speaking because Chrome
   * has a known bug where utterances longer than ~15s silently stall and
   * never fire `onend` unless nudged periodically.
   */
  async speak(
    text: string,
    language: Language,
    ...
    /*
     * Wait for Chrome to populate voices
    ...
    /*
     * ==================================================
     * INDIAN ENGLISH FEMALE VOICE SELECTION
     * ==================================================
     */
    ...
    /*
     * First priority:
     * voices explicitly identified as female.
     */
    ...
    /*
     * Second priority:
     * known Google/Microsoft Indian English voices.
     */
     ...
     * any en-IN voice.
     *
     * This is necessary because browser voice names
     * do NOT always contain "female".
     */
    if (!indianFemaleVoice) {
    ...
    if (indianFemaleVoice) {
      ...
      /*
       * No en-IN voice exists.
       *
       * Do NOT silently use en-US Zira.
       */
      ...
      /*
       * Keep the utterance language as en-IN.
       * Browser may provide its own matching voice.
       */
      u.lang = "en-IN";
    }
    ...
    console.log(
      `[audio] TTS start: "${text.slice(0, 40)}${
        text.length > 40 ? "…" : ""
      }"`
    );
    ...
    /*
     * Small delay makes Chrome more reliable
     * about respecting the selected voice.
     */
    ...
    /*
     * Speak using the selected Indian English voice, and resolve this
     * promise ONLY when speech has genuinely finished. This is the
     * authoritative completion signal the renderer relies on to know an
     * explanation (or the end card) has been spoken in full.
     */
    return new Promise<void>((resolve) => {
      ...
      u.onend = () => {
        console.log("[audio] TTS onend fired — narration complete.");
        finish();
      };

      u.onerror = (e) => {
        console.warn("[audio] TTS onerror:", e.error);
        finish();
      };

      // Chrome-only workaround: long utterances can silently stall without
      // ever firing onend. Nudging pause/resume periodically keeps it alive.
  ...
  /**
   * Defensive fallback only — under normal operation, callers await
   * speak() directly (which resolves on the real onend event), so this
   * should return almost immediately. Kept as a safety net in case a
   * browser fails to fire any completion event at all.
   */
