...
import { downloadBlob, renderVideo, type RenderStage } from "@/lib/quiz/export";
...
export function CsvBatchPanel({ baseQuiz, orientation, audio, audioSettings }: Readonly<Props>) {
  ...
  const generateAll = async () => {
    ...
    setBatchBusy(true);
    // Ask for narration capture once, up front, before the loop — the first
    // renderVideo() call inside the loop still triggers captureNarration()
    // itself (it's idempotent), but doing it here means the user sees the
    // permission prompt immediately instead of partway through row 1's render.
    const narrationOk = await audio.captureNarration();
    if (!narrationOk) {
      toast.warning("Narration capture wasn't granted — batch videos will render with SFX only.");
    }
    for (const video of videos) await generateOne(video);
    setBatchBusy(false);
  };

  const filename = (video: BatchVideo) => `GK_Question_${String(video.number).padStart(2, "0")}.${video.extension ?? "mp4"}`;
    ...
    <section className="space-y-4 rounded-3xl bg-card p-5 shadow-[var(--shadow-soft)] lg:col-span-2">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">One CSV row becomes one independently downloadable quiz video, with narration baked into the MP4.</p>
        </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {videos.map((video) => (
              <article key={video.id} className="space-y-3 rounded-2xl border border-border bg-background p-4">
                ...
                {video.previewUrl && (
                  <video controls preload="metadata" src={video.previewUrl} className="aspect-video w-full rounded-xl bg-black">
                ...
                {video.status === "generating" && (
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-semibold text-muted-foreground">{video.stage ?? "Generating..."}</p>
                  </div>
                )}
...
function Status({ status }: Readonly<{ status: VideoStatus }>) {
  ...
  return <span className={`rounded-full px-2 py-1 text-xs font-bold ${color}`}>{labels[status]}</span>;
}