...
export function CsvBatchPanel({
  ...
  useEffect(
    () => () => {
      videosRef.current.forEach(
        (video) => video.previewUrl && URL.revokeObjectURL(video.previewUrl),
      );
    },
    [],
  );
  ...
  const filename = (video: BatchVideo) =>
    `GK_Quiz_Question_${String(video.number).padStart(2, "0")}.${video.extension ?? "mp4"}`;
  ...
  const downloadAll = async () => {
    ...
    const available = videos.filter(
      (video) => video.status === "completed" && video.blob,
    );
    let processed = 0;
    let completed = 0;
    ...
    toast.success(
      `Download completed. ${completed} / ${videos.length} videos downloaded${skipped ? `; ${skipped} skipped.` : "."}`,
    );
  };
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="flex items-center gap-2 font-display text-xl font-extrabold">
            <FileSpreadsheet className="size-5 text-primary" /> Upload CSV &
            Generate Videos
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            One CSV row becomes one independently downloadable quiz video.
          </p>
        </div>
        <input
          ref={inputRef}
        ...
        <Button variant="hero" onClick={() => inputRef.current?.click()}>
          <Upload /> Upload CSV &amp; Generate Videos
        </Button>
      </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-muted px-4 py-3">
            ...
            <div className="flex gap-2">
              ...
              <Button
                variant="outline"
                disabled={
                  !!downloadState || !videos.some((video) => video.blob)
                }
                onClick={downloadAll}
              >
          ...
          {downloadState && (
            <div className="space-y-2">
              ...
              <p className="text-sm font-bold">
                Downloading videos... {downloadState.current} /{" "}
                {downloadState.total} · {downloadState.name}
              </p>
            </div>
          )}
...
function Status({ status }: { status: VideoStatus }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-1 text-xs font-bold ${status === "failed" ? "bg-destructive/10 text-destructive" : status === "completed" ? "bg-green-100 text-green-700" : "bg-secondary text-secondary-foreground"}`}
    >
      {labels[status]}
    </span>
  );
