...
export function CsvBatchPanel({ baseQuiz, orientation, audio }: Readonly<Props>) {
  ...
  const generateOne = async (video: BatchVideo) => {
    ...
    update(video.id, { status: "generating", percent: 0 });
    try {
      const quiz = csvRowToQuiz(video.row, baseQuiz);
      const result = await renderVideo({
        quiz,
        timeline: buildTimeline(quiz, randomSeed()),
  ...
  const downloadAll = async () => {
    ...
    toast.success(`Download completed. ${completed} / ${videos.length} videos downloaded${suffix}`);
  };
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {videos.map((video) => (
              <article key={video.id} className="space-y-3 rounded-2xl border border-border bg-background p-4">
                ...
                {video.previewUrl && <video controls preload="metadata" src={video.previewUrl} className="aspect-video w-full rounded-xl bg-black"><track kind="captions" src="data:text/vtt,WEBVTT" /></video>}
...
function Status({ status }: Readonly<{ status: VideoStatus }>) {
  ...
  const className = status === "failed" ? "bg-destructive/10 text-destructive" : status === "completed" ? "bg-green-100 text-green-700" : "bg-secondary text-secondary-foreground";
  return <span className={`rounded-full px-2 py-1 text-xs font-bold ${className}`}>{labels[status]}</span>;
}