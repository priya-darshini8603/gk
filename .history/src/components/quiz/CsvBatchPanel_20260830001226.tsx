...
import type { Timeline } from "@/lib/quiz/timeline";
...
export function CsvBatchPanel({
  ...
  /**
   * PERF FIX: previously this ran the CSV batch fully sequentially — for
   * each row, measure narration (Pass 1, real-time/wall-clock bound on
   * SpeechSynthesis) THEN render+encode frames (Pass 2, CPU/GPU bound),
   * one row after another. Total time was
   * numRows × (narration seconds + encode seconds).
   *
   * Those two passes touch different resources: Pass 1 only needs
   * AudioEngine/SpeechSynthesis, Pass 2 only needs the canvas + WebCodecs
   * encoder. So instead of waiting for row N's encoding to finish before
   * starting row N+1's narration measurement, we kick off row N+1's
   * narration measurement AS SOON AS row N's narration measurement (and
   * audio synthesis) finishes — i.e. WHILE row N is still encoding frames.
   *
   *   row N:    [ measure narration ][ render+encode frames ]
   *   row N+1:                       [ measure narration ][ render+encode frames ]
   *                                   ^ starts here, overlapping row N's encode
   *
   * This hides the slowest part of the pipeline (real-time narration
   * measurement) behind the CPU/GPU-bound part instead of paying for both
   * back-to-back on every single row.
   */
  const generateAll = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "This browser can't export video.");
      setBatchBusy(false);
      return;
    }
    ...
    const queue = videosRef.current;

    type Built = { video: BatchVideo; quiz: Quiz; timeline: Timeline; built: FinalAudioTimeline };

    // Measures narration + synthesizes audio for one row. This is the part
    // that's safe to run concurrently with a DIFFERENT row's frame
    // rendering, since it never touches the canvas or the video encoder.
    const measureOne = async (video: BatchVideo): Promise<Built | null> => {
      if (video.previewUrl) {
        URL.revokeObjectURL(video.previewUrl);
      }
      ...
      try {
        ...
        return { video, quiz, timeline, built };
      } catch (error) {
        update(video.id, {
          status: "failed",
          error: error instanceof Error ? error.message : "Rendering failed.",
    ...
    // Kick off measuring the first row before the loop starts.
    let nextMeasured: Promise<Built | null> | null =
      queue.length > 0 ? measureOne(queue[0]!) : null;

    for (let i = 0; i < queue.length; i++) {
      const current = await nextMeasured;

      // Immediately start measuring the NEXT row's narration — this runs
      // concurrently with the frame-rendering/encoding below, since they
      // don't share any resource.
      const upcoming = queue[i + 1];
      nextMeasured = upcoming ? measureOne(upcoming) : null;

      if (!current) {
        // measureOne() already marked this row as failed.
        continue;
      }
      ...
      try {
        const result = await renderVideoFrames({
          ...
          width,
          height,
          onProgress: ({ percent, stage }) => update(current.video.id, { percent, stage }),
        });
